...
int valueof(string str) {
  ...
  for (int i = 0; i < n; i++) {
    int digit =
        (str[i] >= '0' && str[i] <= '9') ? str[i] - '0' : str[i] - 'a' + 10;
...
void load_code(string filename, string filepath) {
  ...
  while (getline(fin, line)) {
    ...
    int start_offset =
        cur.variables.empty() ? 0 : cur.variables.back().end_offset;
...
void load_data(string filename, string filepath) {
  ...
  while (getline(fin, line)) {
    ...
    int start_offset =
        cur.variables.empty() ? cur.offset : cur.variables.back().end_offset;
  ...
  if (!preseted_offset)
    dataseg_offset = cur.variables.empty() ? dataseg_offset
                                           : cur.variables.back().end_offset;
}
...
void load_file(string filepath) {
  ...
  if (filepath.length() < 16 ||
      filepath.substr(filepath.length() - 11, 11) != "_output.txt")
    return;
